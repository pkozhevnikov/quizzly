...
object Quiz:
  ...
  final case class SectionAdded(title: String, sc: SC) extends Event
  final case class UpdateSection(
      sc: SC,
      title: String,
      replyTo: ActorRef[Resp[Nothing]]
  ) extends Command
  final case class SectionUpdated(sc: SC, title: String) extends Event
  final case class RemoveSection(sc: SC, replyTo: ActorRef[Resp[Nothing]]) extends Command

  final case class SaveItem(
      item: Item,
      sectionSC: SC,
      maybeSc: Option[SC] // empty if the item is new
  )
