...
class HttpFrontendSpec
  ...
  val allPersons = Map(pers1.id -> pers1, pers2.id -> pers2)
  ...
  object eaware extends EntityAware:
    ...
    def trial(id: TrialID) = TestEntityRef(
      TrialEntity.EntityKey,
      id,
  ...
  def exam[R](expectedId: ExamID, resp: Resp[R]) =
    new EntityAware:
      val beh = Behaviors.receiveMessage[ExamEntity.Command] {
        case c: ExamEntity.CommandWithReply[R] =>
          c.replyTo ! resp
          Behaviors.stopped
      }
      def exam(id: String) =
      ...
      def trial(id: String) = TestEntityRef(
        TrialEntity.EntityKey,
        id,
        testKit.spawn(Behaviors.empty)
      )
  ...
  "{id}" when {
    ...
    "PATCH" should {
      ...
      "respond with error" in {
        patch("exam-1", pers1) ~> stdtrial("pers1-exam-1", Bad(notTestee.error())) ~>
          check {
            status shouldBe StatusCodes.UnprocessableEntity
      ...
      "return start details" in {
        ...
        patch("exam-1", pers1) ~> stdtrial("pers1-exam-1", beh) ~>
          check {
            status shouldBe StatusCodes.OK
    ...
    "POST" should {
      ...
      "respond with error" in {
        ...
        post("t1", pers1, subm) ~> stdtrial("t1", Bad(trialFinalized.error())) ~>
          check {
            status shouldBe StatusCodes.UnprocessableEntity
      ...
      "return submission result" in {
        ...
        val subm = Solution("i1", List("a1", "a2"))
        post("t1", pers1, subm) ~> stdtrial("t1", beh) ~>
          check {
            status shouldBe StatusCodes.OK
